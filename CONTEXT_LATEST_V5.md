...
## Milestone 6: Development Helper Endpoint (Completed)

Status:
- Added a development-only endpoint for frontend discovery of seeded demo entities
- Route registration is gated so helper routes are not available in production
...
Behavior:
- GET /api/dev/demo-context returns demo user and demo portfolio context with holdings
- Returns a standard 404 envelope if demo entities are missing
- Endpoint is intentionally unavailable when NODE_ENV=production

## Milestone 7: Portfolio Analysis Orchestration Endpoint (Completed)

Status:
- Added orchestration service for portfolio-wide analysis execution
- Added API endpoint for one-call analysis run from frontend
- Added unit and integration test coverage
- Updated API docs with usage example

Files:
- src/services/portfolio-analysis.service.ts
- src/types/services.ts
- src/services/index.ts
- src/api/routes/portfolios.routes.ts
- tests/unit/portfolio-analysis.service.test.ts
- tests/integration/api-portfolio-analysis.integration.test.ts
- docs/api.md

Endpoint:
- POST /api/portfolios/:portfolioId/run-analysis

Orchestration behavior:
- Validates portfolioId input
- Loads portfolio overview and fails with not found semantics when absent
- Iterates holdings and runs deterministic ticker report generation per holding
- Continues on per-ticker failures (records failed ticker + reason)
- Aggregates counts for holdings analyzed, reports created, predictions created
- Generates and returns portfolio summary when appropriate
- Returns execution timestamps (startedAt, finishedAt)
...
- Health endpoints
- Main route groups
- Example curl commands including run-analysis and dev demo-context
...
- Portfolio analysis orchestration endpoint
