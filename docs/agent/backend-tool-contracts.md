...
| `getDiscoveryCandidates` | Discovery Read | `READ_ONLY` | `AUTO_ALLOWED` | `category` | Executes normally | `category`, `candidateCount`, `topTickers` | Snapshot quality/freshness depends on prior discovery refresh |
| `getGeopoliticalSummary` | Geopolitical Read | `READ_ONLY` | `AUTO_ALLOWED` | none | Executes normally | `totalEvents`, `sentimentMix`, `topHeadlines`, `topRisks` | Depends on prior GDELT ingestion |
...
| `refreshGdeltRiskContext` | Geopolitical Refresh | `REFRESH` | `CONFIRMATION_REQUIRED` | none | Planned action only, no writes/provider calls | `plannedOrExecuted`, `queriesProcessed`, `queriesFailed`, `eventsCreated`, `warningCount` | Query throttling/partial failures may occur |
